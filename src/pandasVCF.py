...
class Vcf(object):
    ...
    def __init__(self, filename, sample_id='all', cols=['#CHROM', 'POS', 'REF', 'ALT', 'FORMAT'],
        ...
        self.vcf_chunks = pd.read_table(filename, sep="\t", compression=header_parsed.compression,
                                        skiprows=(len(self.header_df) - 2), usecols=self.usecols, chunksize=chunksize)
    ...
    def add_variant_annotations(self, split_columns='', verbose=False, inplace=False, drop_hom_ref=True):
        ...
        df_vcf_cols = self.df[list(set(self.df.columns) - set(['CHROM', 'POS', 'REF', 'ALT', 'FORMAT']) - set(self.sample_id))]

        if self.n_cores == 1:
            if inplace:
                self.df = process_variant_annotations([self.df, split_columns, self.sample_id, self.drop_hom_ref])
                self.df = self.df.join(df_vcf_cols)  # joining QUAL, FILTER, and/or INFO columns
            else:
                self.df_annot = process_variant_annotations([self.df, split_columns, self.sample_id, self.drop_hom_ref])

        else:
            var_annot = mp_variant_annotations(self.df.copy(), split_columns, self.sample_id,
                                               self.drop_hom_ref, self.n_cores)

            if inplace:
                self.df = var_annot.join(df_vcf_cols)  # joining QUAL, FILTER, and/or INFO columns
            else:
