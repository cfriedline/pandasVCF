...
def vartype_map(ref_alt_bases):
    ...
    if ref == alt:
        return 'ref'  # Orderde by frequency of the variant to reduce complexity
...
def process_variant_annotations(df_vars_split_cols_sample_id_drop_hom_ref):
    ...
    if len(parsed_df) > 0:
        ...
        df_annot.reset_index('sample_ids', drop=False, inplace=True)  # reseting sample_ids from index
        return df_annot
    else:
...
def get_vcf_annotations(df, sample_name, split_columns='', drop_hom_ref=True):
        ...
        while len(df) + len(multidf) > 0:
            ...
            df['vartype1'] = list(map(vartype_map, df[['REF', 'a1']].values))
            df['vartype2'] = list(map(vartype_map, df[['REF', 'a2']].values))
